...
function updDist() {
	...
	if (_dstL1) distÅCrn(_dstL1, _tLP, _tRP, _bRP, _bLP)
	if (_dstL2) distÅCrn(_dstL2, _tLP, _tRP, _bRP, _bLP)
	if (_dstL3) distÅCrn(_dstL3, _tLP, _tRP, _bRP, _bLP)
	if (_dstL4) distÅCrn(_dstL4, _tLP, _tRP, _bRP, _bLP)
}
...
function set_aplyBB(a) {
	...
	let œ = returnBoundingBoxCornerPoints(_dstL1)
	_tLP = œ.tLP
	_tRP = œ.tRP
	_bLP = œ.bLP
	_bRP = œ.bRP
...
function ____________________________________________MISC() {}
 ...
 function µ(n, iMn, iMx, oMn, oMx, œ) {
	/* Map Function - v2017-07-31
	{çO:1} clamp to output range, {çI:1} clamp to input range,
	{unÇIs:1} return value when outside clamped range,
	{dv0is:1} return 1 instead of 0 when dividing by 0 */
	var œ = œ || {}
	if (œ.çI) {
		if (iMn < iMx) {
			if (œ.unÇIs !== undefined && œ.unÇIs !== null) {
				if (n < iMn || n > iMx) return œ.unÇIs
			}
			n = ç(n, iMn, iMx)
		} else {
			if (œ.unÇIs !== undefined && œ.unÇIs !== null) {
	...
	var dvsr = (iMx - iMn)
	if (dvsr === 0) return œ.dv0is ? œ.dv0is : 0
	if (oMx - oMn === 0) return oMx
	n = oMn + ((oMx - oMn) * ((n - iMn) / dvsr))
	if (œ.çO) {
		if (oMn < oMx) {
			if (œ.unÇIs !== undefined && œ.unÇIs !== null) {
				if (n < oMn || n > oMx) return œ.unÇIs
			}
			n = ç(n, oMn, oMx)
		} else {
 ...
 function ç(n, mn, mx, usesAutoInvert) {
 ...
 function crvTF(vlu, srt, end, tF, unclmp) {
	...
	var n = µ(vlu, srt, end, 0, 1)
	if (!unclmp) n = ç(n, 0, 1)
	tF = nTF(tF, null, null, null, "crvTF")
	n = objc_msgSend((tF), "_solveForInput:", n)
	return µ(n, 0, 1, srt, end)
}

 function nTF(a, b, c, d, ßTag) {
 ...
 function typOf(a, retLongName, ßTag) {
 ...
 function isŒ(œ) {
	return œ && typeof œ === 'object' && œ.constructor === Object;
}

 function tFToÆ(tfCrv) {
	/* Timing Function To Array v2017-06-14 */
	return new Array(tfCrv).toString()
		.replace("TimingFunction(", "").replace(")", "").split(",")
		.map(function(ﬆ) { return parseFloat(ﬆ) })
}

 function aplyPrspDist(l, x, y, rset) {
	...
	let t = rset ? new Transform3D : l.transform
	Transaction.disableActions = true
	l.setValueForKeyPath(l.transform = new Transform3D(
 ...
 function distÅCrn(l, tLP, tRP, bRP, bLP) {
	...
	vFK(l, "transform", Transform3D.identity())
	...
	let t = distRctCrn(l.bounds, box, tLP, tRP, bLP, bRP)
	...
	l.transform = trnslt1.concat(t).concat(trnslt2)
}
 ...
 function distRctCrn(rct, box, tLP, tRP, bLP, bRP) {
	...
	translated from swift function (altered for un-flipped geom) */
	...
	*/
	...
	if (v === undefined) {
		...
		if (k[0] === "G" && k[3] === "P"){
			...
			let gP = œ.superlayer.convertPointToLayer(œ.position, rL)
			v = k[4] === X ? gP.x : k[4] === Y ? gP.y : gP
		}
		else return œ.valueForKeyPath(k)
	}
	...
	//--send value mode
		...
		if (k === "RotY") {
			œ.transform = Transform3D.identity().rotate(v, 0, 1, 0)
		}
		//--set value directly to key
		else if (safe) try { œ.setValueForKeyPath(v, k); } catch (err) {}
		else œ.setValueForKeyPath(v, k)
	}
}
